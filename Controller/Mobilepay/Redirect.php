...
namespace PayEx\Payments\Controller\Mobilepay;
...
class Redirect extends \Magento\Framework\App\Action\Action
{
     ...
     * @var array
     */
    private $additionalParams = ['RESPONSIVE'=>1, 'USEMOBILEPAY'=>'True'];
    ...
    public function execute()
    {
        ...
        if ($order_id) {
            ...
            $this->session->restoreQuote();
            $this->messageManager->addError(__('Invalid order_id value'));
            $this->_redirect('checkout/cart');
            return;
        }
        ...
        $additional = $this->prepareAdditionalParams();
    ...
    private function prepareAdditionalParams()
    {
        ...
        foreach ($this->additionalParams as $key => $value) {
            if ($count < (sizeof($this->additionalParams)-1)){
                $result .= $key.'='.$value;
                $result .= '&';
            } else {
                $result .= $key.'='.$value;
